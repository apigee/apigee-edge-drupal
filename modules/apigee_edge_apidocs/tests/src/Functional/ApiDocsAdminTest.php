...
class ApiDocsAdminTest extends BrowserTestBase {
  ...
  public function testApiDocAdministration() {
    ...
    $page->fillField('name[0][value]', 'API-Doc-A');
    $page->fillField('description[0][value]', 'API-Doc-A Description');
    ...
    $assert->pageTextContains('Created the API-Doc-A API Doc.');

    // Entity listed.
    $assert->linkExists('API-Doc-A');
    $assert->linkExists('Edit');
    $assert->linkExists('Delete');

    // Click on API Doc to edit.
    $this->clickLink(t('API-Doc-A'));
    $assert->statusCodeEquals(200);

    // Edit form should have proper values.
    $assert->fieldValueEquals('name[0][value]', 'API-Doc-A');
    $assert->fieldValueEquals('description[0][value]', 'API-Doc-A Description');
    $assert->fieldValueEquals('status[value]', '1');
